"""Calibration core — Brier/ECE scoring and recalibration.

VENDORED from ``csj571/bre-sim`` ``engine/calibration.py`` (the BRE engine's
BUILD_PLAN B0 module). That repo is the source of truth for this math; make
edits there and re-copy, so any drift between the two copies stays visible in
this note and in the diff. Only this docstring differs from the upstream file.

Hand-rolled in NumPy only (no sklearn/scipy/torch). Everything here scores a
set of probabilistic predictions against resolved binary outcomes:
